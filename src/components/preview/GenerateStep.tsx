...
import { loadPDF } from '@/engine/pdf-processor'
import { useAppStore } from '@/store/use-app-store'
import { GenerationPhase } from '@/types/kdp'
...
export default function GenerateStep({
      ...
      if (coverData) {
        try {
          ...
          setGenerationProgress({
            ...generationProgress,
            progress: 75,
            details: segments.isFullSpread ? 'Full cover spread detected and sliced' : 'Front cover image prepared',
          })
        } catch (err) {
  ...
  return (
    <div className="ds-page-stage flex h-full min-h-0 items-center justify-center overflow-hidden">
          ...
          >
            ...
            <div className="text-center">
              <p className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-primary">
                {isError ? 'Generation stopped' : isComplete ? 'Generation complete' : 'Preview generation'}
              </p>
            ...
            {!isError && (
              <div className="rounded-2xl border border-border bg-surface p-4 shadow-soft">
                <div className="flex justify-between text-xs">
                  <span className="font-semibold text-foreground">{generationProgress.phaseLabel}</span>
                  <span className="font-mono font-bold text-primary">{generationProgress.progress}%</span>
                </div>
                <Progress value={generationProgress.progress} className="mt-3 h-2.5 bg-secondary [&>div]:bg-primary" />
                {generationProgress.details && (
                  <p className="mt-3 text-xs text-muted-foreground">{generationProgress.details}</p>
                )}
              </div>
            )}
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <Button
                  onClick={() => {
                    ...
                    setGenerationProgress({ phase: 'idle', phaseLabel: '', progress: 0 })
                    generate3DPreview()
                  }}
    ...
    >
      ...
      <span
        className={`text-xs font-mono ${
          status === 'active' ? 'text-primary' : status === 'complete' ? 'text-success/70' : 'text-muted-foreground/35'
        }`}
      >
